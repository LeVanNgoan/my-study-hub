  ...
  return <>
    ...
    {!data.current_semester ? (
      ...
      <div className="two-col">
        <div><SectionTitle title="Recent study"/><div className="list-card">{data.recent_notes.length ? data.recent_notes.map(n=><NoteRow key={n.id} note={n} onClick={()=>onOpenSubject(n.subject_id)}/>) : <SmallEmpty text="No study notes yet."/>}</div></div>
        <div><SectionTitle title="Need review"/><div className="list-card">{data.need_review.length ? data.need_review.map(n=><NoteRow key={n.id} note={n} onClick={()=>onOpenSubject(n.subject_id)} review/>) : <SmallEmpty text="Nothing waiting for review."/>}</div></div>
      </div>
    </>}
  </>;
}
